...
class EpidemicSceneGPU:
    ...
    def get_statis(self):
        ...
        for i in range(0,self.current_step):
            ...
            for k in range(0,len(self.node_name)):
                ...
                now_data[name]=int(100*self.step_infected_status_cnt[i][k])
            ans0[str(i)]=now_data
        for i in range(0,self.current_step):
            now_data = {}
            for k in range(0, len(self.edges)):
                ...
                now_data[name0+'->'+name1] = int(100*self.step_status_trasition_cnt[i][node0][node1])
            ans1[str(i)] = now_data
        ans = {}
        ans['status']=ans0
        ans['trasition'] = ans1
    ...
    def simulation(self,total_step):
        ...
        #开启线程监测内存和显存情况
        self.thread0=threading.Thread(target=self.simulation_thread)
...
class GeoInfo:
    ...
    def data_to_numpy(self):   #根据GIS数据生成必要信息，并将数据组织为numpy（目前无真实数据输入，随意生成）
        ...
        for i in range(0,math.floor(0.5*self.n_buildings)):
            lat=min_lat+Dlat*random.random()
            lon=min_lon+Dlon*random.random()
            index_lat=math.floor((lat-min_lat)/dlat)
            index_lon=math.floor((lon-min_lon)/dlon)
...
def construct_scene_temp():
    ...
    infected_model.set_model(
        [{'state0': '易感', 'state1': '感染', 'p0': '1', 'p1': '1', 'p2': '1', 't0': '1', 't1': '1', 't2': '1'},
         {'state0': '感染', 'state1': '康复', 'p0': '1', 'p1': '1', 'p2': '1', 't0': '120', 't1': '120', 't2': '120'}])
...
def get_heats_3d():
    global scene
    res=scene.get_heats(scene)
    heats=res['heats']
    now_step=res['now_step']
    ans0={}
    ans1={}
    for key0 in heats.keys():
        ...
        for name in scene.node_name:
            if name not in heats.keys():
                ans0[name] = []
                ans1[name] = []
    return json.dumps({'heat_data':ans0,'grid_data':ans1,'now_step':now_step})
...
@app.route("/get_grid_step",methods=['GET'])
def get_grid_step():
    step = int(request.values['step'])
    heats = scene.get_heats_step(step)
    ans1 = {}
